...
// POST /api/cron/monthly-digest — email a monthly (30-day) performance summary
// to every active workspace's admins. Bearer CRON_SECRET guarded.
export async function POST(request: NextRequest) {
  ...
  for (const ws of (workspaces ?? []) as Array<{ id: string; name: string | null }>) {
    const r = await sendWeeklyDigest(db, ws, 'month');
