...
export async function executeCampaign(campaignId: string): Promise<CampaignRunResult> {
  ...
  // Some inbound auto-replies arrive before the campaign_recipients row is committed.
  // This retroactively links any missed replies via the shared, send-time-guarded rule,
  // so it can never attribute a reply that predates its send and never double-counts a
  // reply across overlapping campaigns. The webhook is the primary, real-time path.
  void backfillCampaignReplies(db, campaignId, campaign.workspace_id)
    .then((n: number) => { if (n > 0) console.log(`[Campaign ${campaignId}] Post-sync: +${n} replies linked`); })
    .catch((e: unknown) => console.error(`[Campaign ${campaignId}] Post-completion reply sync failed:`, e));
