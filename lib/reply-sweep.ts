...
async function record(supabase: any, row: SweepRow, body: string, wamid: string): Promise<void> {
...
export async function sendCatchupReply(supabase: any, row: SweepRow): Promise<'sent' | 'skipped' | 'failed'> {
  try {
    ...
    if (bh?.is_enabled && !isWithinBusinessHours(bh as BusinessHoursConfig)) {
      ...
      await record(supabase, row, bh.away_message, wamid);
      return 'sent';
    }
    ...
    // Conversation history (last 40, oldest→newest) for context.
    ...
    if (!wamid) return 'failed';
    await record(supabase, row, reply, wamid);
    return 'sent';
  } catch (e) {
