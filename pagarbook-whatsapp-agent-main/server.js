...
async function ensureNewTables() {
  ...
  try {
    ...
    `);
    console.log('[DB] New feature tables ensured (inbox_rules, sequences, leads)');
  } catch(e) { console.error('[DB] ensureNewTables error:', e.message); }
  finally { c.release(); }
}
ensureNewTables();

// Inbox Rules
app.get('/api/inbox-rules', async (req, res) => {
  const { client_id } = req.query;
  if (!client_id) return res.status(400).json({ error:'client_id required' });
  try {
    const c = await db.getCentralClient();
    const result = await c.query('SELECT * FROM public.inbox_rules WHERE client_id=$1 ORDER BY priority DESC, created_at DESC', [client_id]);
    c.release();
    res.json(result.rows);
...
app.post('/api/inbox-rules', async (req, res) => {
  const { client_id, name, trigger_type, trigger_value, actions, priority, is_active } = req.body;
  if (!client_id || !name || !trigger_type) return res.status(400).json({ error:'client_id, name, trigger_type required' });
  try {
    const c = await db.getCentralClient();
    const result = await c.query(
      'INSERT INTO public.inbox_rules (client_id,name,trigger_type,trigger_value,actions,priority,is_active) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *',
      [client_id, name, trigger_type, trigger_value||{}, actions||[], priority||0, is_active!==false]
    );
    c.release();
    res.status(201).json(result.rows[0]);
  } catch(e) { res.status(500).json({ error:e.message }); }
...
app.put('/api/inbox-rules/:id', async (req, res) => {
  const { name, trigger_type, trigger_value, actions, priority, is_active } = req.body;
  try {
    const c = await db.getCentralClient();
    const result = await c.query(
      'UPDATE public.inbox_rules SET name=$1,trigger_type=$2,trigger_value=$3,actions=$4,priority=$5,is_active=$6 WHERE id=$7 RETURNING *',
      [name, trigger_type, trigger_value||{}, actions||[], priority||0, !!is_active, req.params.id]
    );
    c.release();
    res.json(result.rows[0]);
  } catch(e) { res.status(500).json({ error:e.message }); }
...
app.delete('/api/inbox-rules/:id', async (req, res) => {
  try {
    const c = await db.getCentralClient();
    await c.query('DELETE FROM public.inbox_rules WHERE id=$1', [req.params.id]);
    c.release();
    res.json({ success:true });
  } catch(e) { res.status(500).json({ error:e.message }); }
});
...
app.get('/api/sequences', async (req, res) => {
  ...
  try {
    const c = await db.getCentralClient();
    const result = await c.query('SELECT * FROM public.follow_up_sequences WHERE client_id=$1 ORDER BY created_at DESC', [client_id]);
    c.release();
    res.json(result.rows);
  } catch(e) { res.status(500).json({ error:e.message }); }
});
...
app.post('/api/sequences', async (req, res) => {
  const { client_id, name, steps, is_active } = req.body;
  if (!client_id || !name) return res.status(400).json({ error:'client_id and name required' });
  try {
    const c = await db.getCentralClient();
    const result = await c.query(
      'INSERT INTO public.follow_up_sequences (client_id,name,steps,is_active) VALUES ($1,$2,$3,$4) RETURNING *',
      [client_id, name, JSON.stringify(steps||[]), is_active!==false]
    );
    c.release();
    res.status(201).json(result.rows[0]);
  } catch(e) { res.status(500).json({ error:e.message }); }
...
app.put('/api/sequences/:id', async (req, res) => {
  ...
  try {
    const c = await db.getCentralClient();
    const result = await c.query(
      'UPDATE public.follow_up_sequences SET name=$1,steps=$2,is_active=$3 WHERE id=$4 RETURNING *',
      [name, JSON.stringify(steps||[]), !!is_active, req.params.id]
    );
    c.release();
    res.json(result.rows[0]);
  } catch(e) { res.status(500).json({ error:e.message }); }
...
app.delete('/api/sequences/:id', async (req, res) => {
  try {
    const c = await db.getCentralClient();
    await c.query('DELETE FROM public.follow_up_sequences WHERE id=$1', [req.params.id]);
    c.release();
    res.json({ success:true });
  } catch(e) { res.status(500).json({ error:e.message }); }
});
...
app.post('/api/sequences/:id/enroll', async (req, res) => {
  ...
  try {
    ...
    for (const phone of phones) {
      await c.query(
        'INSERT INTO public.sequence_enrollments (sequence_id,contact_phone,status) VALUES ($1,$2,$3) ON CONFLICT DO NOTHING',
        [req.params.id, phone, 'active']
      );
      enrolled++;
    }
    c.release();
...
app.get('/api/leads', async (req, res) => {
  ...
  try {
    const c = await db.getCentralClient();
    const result = await c.query(
      'SELECT l.*, a.full_name as agent_name FROM public.leads l LEFT JOIN public.agents a ON a.id=l.assigned_agent_id WHERE l.client_id=$1 ORDER BY l.created_at DESC',
      [client_id]
    );
    c.release();
    res.json(result.rows);
...
app.post('/api/leads', async (req, res) => {
  const { client_id, title, contact_phone, stage, value, priority, notes, follow_up_at, assigned_agent_id } = req.body;
  if (!client_id || !title) return res.status(400).json({ error:'client_id and title required' });
  try {
    const c = await db.getCentralClient();
    const result = await c.query(
      'INSERT INTO public.leads (client_id,title,contact_phone,stage,value,priority,notes,follow_up_at,assigned_agent_id) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *',
      [client_id, title, contact_phone||null, stage||'new', value||null, priority||'medium', notes||null, follow_up_at||null, assigned_agent_id||null]
    );
    c.release();
    res.status(201).json(result.rows[0]);
  } catch(e) { res.status(500).json({ error:e.message }); }
...
app.patch('/api/leads/:id', async (req, res) => {
  const fields = ['title','stage','value','notes','priority','follow_up_at','assigned_agent_id'];
  ...
  try {
    const c = await db.getCentralClient();
    const result = await c.query(`UPDATE public.leads SET ${updates.join(',')},updated_at=NOW() WHERE id=$${vals.length} RETURNING *`, vals);
    c.release();
    res.json(result.rows[0]);
  } catch(e) { res.status(500).json({ error:e.message }); }
});
...
app.delete('/api/leads/:id', async (req, res) => {
  try {
    const c = await db.getCentralClient();
    await c.query('DELETE FROM public.leads WHERE id=$1', [req.params.id]);
    c.release();
    res.json({ success:true });
  } catch(e) { res.status(500).json({ error:e.message }); }
});
